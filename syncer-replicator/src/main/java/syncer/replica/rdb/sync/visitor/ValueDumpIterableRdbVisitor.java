package syncer.replica.rdb.sync.visitor;

import syncer.replica.event.Event;
import syncer.replica.io.RedisInputStream;
import syncer.replica.rdb.BaseRdbParser;
import syncer.replica.rdb.DefaultRedisRdbVisitor;
import syncer.replica.rdb.RedisRdbValueVisitor;
...
/**
 * @author zhanenqiang
 * @Description 除LIST HASH SET ZSET 命令外其他基于dump restore的方式
 * @Date 2020/12/22
 */
public class ValueDumpIterableRdbVisitor extends DefaultRedisRdbVisitor {


    public ValueDumpIterableRdbVisitor(Replication replication) {
        this(replication, new ValueIterableDumpRdbValueVisitor(replication));
    }

    public ValueDumpIterableRdbVisitor(Replication replication, RedisRdbValueVisitor valueParser) {
        super(replication, valueParser);
    }

    public ValueDumpIterableRdbVisitor(Replication replication, int version) {
        this(replication, version, 8192);
    }


    public ValueDumpIterableRdbVisitor(Replication replication, int version, int size) {
        super(replication, new ValueIterableDumpRdbValueVisitor(replication, version, size));
    }
