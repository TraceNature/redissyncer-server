 ...
 */
@Slf4j
public class MulitJedisPipeLineClient implements JDRedisClient {
...
//    private  Map<String,Integer>incrMap= new LruCache<>(1000);
    ...
    private AtomicBoolean nodeSuccessStatus;
    private AtomicInteger nodeSuccessStatusType;
    private CommandCompensatorUtils commandCompensatorUtils = new CommandCompensatorUtils();

    public MulitJedisPipeLineClient(String host, Integer port, String password, int count, long errorCount, String taskId,String nodeId,String parentNodeId,AtomicBoolean nodeSuccessStatus,AtomicInteger nodeSuccessStatusType) {
        ...
        ThreadPoolUtils.execute(new MulitJedisPipeLineClient.PipelineSubmitThread(taskId));
    }
...
//            log.warn("key[{}]同步失败被抛弃",eventEntity.getStringKey());
    ...
    void compensator(EventEntity eventEntity) {
        if ((MultiSyncTaskManagerutils.isTaskClose(taskId) && taskId != null&&nodeSuccessStatus.get()&&nodeSuccessStatusType.get()==1)||nodeSuccessStatusType.get()==-1) {
            return;
        }
        commitLock.lock();
        try {
...
//                    long error = errorNums.incrementAndGet();
                    long error=MultiSyncTaskManagerutils.get(taskId,nodeId).getErrorNums().incrementAndGet();
...
//            log.error("数据补偿记录写入失败[{}]:[{}]",Strings.byteToString(data.getCmd()),data.getStringKey());
    ...
    class PipelineSubmitThread implements Runnable {
        ...
        public void run() {
            ...
            long stopTime=1000 * 60 * 1;
            while (true) {
                compensatorLock.lock();
                try {
                    submitCommandNum();
//                    if(!nodeSuccessStatus.get()){
//
//                    }

                    if ((MultiSyncTaskManagerutils.isTaskClose(taskId) && taskId != null&&nodeSuccessStatusType.get()==1)||nodeSuccessStatusType.get()==-1) {
                        log.warn("双向同步taskId[{}]节点parentNode节点[{}]子node节点[{}]数据传输模块进入关闭保护状态,不再接收新数据",taskId, parentNodeId,nodeId);
                        Date time = new Date(date.getTime());
                        if (status) {

                            while (System.currentTimeMillis() - time.getTime() <stopTime) {
                                submitCommandNum();
                                try {
                                    Thread.sleep(1000);
                            ...
                            status = false;
                            addCommandNum();
                            log.warn("双向同步taskId[{}]节点parentnode节点[{}]子node节点[{}]数据传输模保护状态退出,任务停止",taskId, parentNodeId,nodeId);
                            try {
                                pipelined.close();
                                jedisPool.close();
                            } catch (Exception e) {
