...
import com.i1314i.syncerplusservice.task.BatchedKeyValueTask.single.BatchedKVSyncTask;
...
public class IRedisReplicatorServiceImpl implements IRedisReplicatorService {
    ...
    public void backupAof(String redisPath, String aofPath) {
        ...
        @Override public void handle(byte... rawBytes) {
        try {
        out.write(rawBytes);
...
//                log.info("同步不同版本（）版本数据...");
    ...
    void batchedSyncToSingle(RedisClusterDto clusterDto) throws TaskMsgException {
        ...
        for (String source : sourceRedisUris
            ...
            try {
                if (i == 0) {
                    batchedSyncTask(syncDataDto, source, String.valueOf(targetRedisUris.toArray()[0]), true);
                } else {
                    batchedSyncTask(syncDataDto, source, String.valueOf(targetRedisUris.toArray()[0]), false);
                }
                i++;
...
//                log.info("同步不同版本（）版本数据...");
    ...
    void BatchedSyncToCluster(RedisClusterDto clusterDto) throws TaskMsgException {
        Set<String> sourceRedisUris = clusterDto.getSourceUris();
        Set<String> targetRedisUris = clusterDto.getTargetUris();
...
//        }
     ...
     * @param clusterDto
     */
    void batchedSyncSingleToCluster(RedisClusterDto clusterDto) throws TaskMsgException {
        Set<String> sourceRedisUris = clusterDto.getSourceUris();
        Set<String> targetRedisUris = clusterDto.getTargetUris();
...
//                log.info("同步不同版本（）版本数据...");
     ...
     * zzj add 提出判断连接的部分
     * */
    public void batchedSyncTask(RedisSyncDataDto syncDataDto, String sourceUri, String targetUri, boolean status) throws TaskMsgException {
        ...
        log.info("-----其他版本");
        threadPoolTaskExecutor.execute(new BatchedKVSyncTask(syncDataDto));
