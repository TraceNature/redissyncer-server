...
public class RedisInputStream extends InputStream {
    ...
    /**
     * @param rawByteListenerList raw byte listeners
     * @since 2.2.0
     */
    public synchronized void setRawByteListenerList(List<TaskRawByteListener> rawByteListenerList) {
        this.rawByteListenerList = rawByteListenerList;
    }

    protected void notify(byte... bytes) {
        if (rawByteListenerList == null || rawByteListenerList.isEmpty()) {
            return;
        }
        for (TaskRawByteListener listener : rawByteListenerList) {
            listener.handler(bytes);
        }
    }
