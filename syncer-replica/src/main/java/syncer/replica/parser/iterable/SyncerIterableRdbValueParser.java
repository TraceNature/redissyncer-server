 ...
 * 切分 list set hash结构
 */
public class SyncerIterableRdbValueParser extends DefaultRedisRdbValueParser {

    public SyncerIterableRdbValueParser(Replication replication) {
        super(replication);
    }
        ...
        public byte[] next() {
            try {
                if (zllen == -1 && condition > 0) {
                    this.stream = new RedisInputStream(parser.rdbGenericLoadStringObject(Constants.RDB_LOAD_NONE));
                    // zlbytes
                    BaseRdbParser.LenHelper.zlbytes(stream);
                    // zltail
                    BaseRdbParser.LenHelper.zltail(stream);
