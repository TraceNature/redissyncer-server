...
import syncer.replica.datatype.command.common.ExpireCommand;
 ...
 * redis>
 */
public class ExpireCommandParser implements CommandParser<ExpireCommand> {
    @Override
    public ExpireCommand parse(Object[] command) {
        int idx = 1;
        byte[] key = CommandParsers.toBytes(command[idx]);
        idx++;
        int ex = CommandParsers.toInt(command[idx++]);
