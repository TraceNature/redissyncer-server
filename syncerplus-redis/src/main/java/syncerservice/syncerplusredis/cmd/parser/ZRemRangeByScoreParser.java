...
import syncerservice.syncerplusredis.cmd.impl.ZRemRangeByScoreCommand;
...
public class ZRemRangeByScoreParser implements CommandParser<ZRemRangeByScoreCommand> {
    ...
    public ZRemRangeByScoreCommand parse(Object[] command) {
        ...
        idx++;
        byte[] min = CommandParsers.toBytes(command[idx]);
        idx++;
        byte[] max = CommandParsers.toBytes(command[idx]);
        idx++;
